...
DEB_FILE="omniapi-with-sdk-2026.0.1.21-amd64.deb"
...
case $choice in
    1)
        ...
        echo "  $DOCKER_COMPOSE_CMD -f docker-compose.test.yml exec omniapi-test bash"
        echo ""
        echo "Để xem logs:"
    ...
    2)
        ...
        echo ""
        echo "Để chạy container:"
        echo "  docker run -it --privileged --name omniapi-test \\"
        echo "    -v /sys/fs/cgroup:/sys/fs/cgroup:ro \\"
        echo "    -p 8080:8080 \\"
        echo "    omniapi-test:latest"
        echo ""
        echo "Hoặc chạy ở background:"
        echo "  docker run -d --privileged --name omniapi-test \\"
        echo "    -v /sys/fs/cgroup:/sys/fs/cgroup:ro \\"
        echo "    -p 8080:8080 \\"
        echo "    omniapi-test:latest"
        echo ""
        echo "Để vào container:"
        echo "  docker exec -it omniapi-test bash"
        ;;
    3)
        echo ""
        echo "=== Build image ==="
        ...
        echo "✓ Build thành công!"
        echo ""
        echo "Image đã được tạo: omniapi-test:latest"
...
echo "   dpkg -l | grep omniapi"
echo ""
echo "2. Kiểm tra executable:"
echo "   ls -la /usr/local/bin/omniapi"
echo ""
echo "3. Kiểm tra service file:"
echo "   cat /etc/systemd/system/omniapi.service"
echo ""
echo "4. Test chạy trực tiếp (không dùng systemd):"
echo "   /usr/local/bin/omniapi --help"
echo ""
echo "5. Test với systemd (trong container):"
echo "   systemctl daemon-reload"
echo "   systemctl start omniapi"
echo "   systemctl status omniapi"
echo ""
