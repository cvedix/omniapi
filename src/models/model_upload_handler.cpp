...
bool ModelUploadHandler::isValidModelFile(const std::string &filename) const {
  // Allow .onnx, .rknn, .weights, .cfg, .pt, .pth, .pb, .pbtxt, .tflite, .txt
  // files .txt files are used for labels/classes (e.g., coco_80classes.txt,
  ...
  return hasSuffix(lower, ".onnx") || hasSuffix(lower, ".rknn") ||
         hasSuffix(lower, ".weights") || hasSuffix(lower, ".cfg") ||
         hasSuffix(lower, ".pt") || hasSuffix(lower, ".pth") ||
         hasSuffix(lower, ".pb") || hasSuffix(lower, ".pbtxt") ||
         hasSuffix(lower, ".tflite") || hasSuffix(lower, ".txt");
}

std::string
ModelUploadHandler::sanitizeFilename(const std::string &filename) const {
  ...
  try {
    ...
    if (isMultipart) {
      ...
      while (partStart != std::string::npos) {
        ...
        try {
          ...
          fileCount++;
          std::cerr << "[ModelUploadHandler] Model file uploaded: "
      ...
      if (fileCount > 0) {
        ...
        response["message"] =
            "Uploaded " + std::to_string(fileCount) + " model file(s)";
      ...
      if (!isValidModelFile(originalFilename)) {
        callback(createErrorResponse(
            ...
            "Only model files (.onnx, .weights, .cfg, .pt, .pth, .pb, .pbtxt, "
            ".tflite, .txt) are allowed"));
        return;
      }
      ...
      if (std::filesystem::exists(filePath)) {
        ...
        std::cerr
            << "[ModelUploadHandler] File with original name exists, using: "
    ...
    try {
      ...
      try {
        ...
        std::cerr << "[ModelUploadHandler] Error getting file size: "
                  << e.what() << std::endl;
      ...
      try {
        ...
        std::cerr << "[ModelUploadHandler] Error getting canonical path: "
                  << e.what() << std::endl;
      ...
      std::cerr << "[ModelUploadHandler] Unexpected error: " << e.what()
                << std::endl;
      callback(createErrorResponse(
...
void ModelUploadHandler::renameModel(
  ...
  try {
    ...
    if (!isValidModelFile(newName)) {
      callback(createErrorResponse(
          400, "Invalid file type",
          "New name must have a valid model file extension (.onnx, .weights, "
          ".cfg, .pt, .pth, .pb, .tflite)"));
      return;
    }
    ...
    if (!std::filesystem::exists(sourcePath)) {
      callback(
          createErrorResponse(404, "Not found", "Source model file not found"));
      return;
    }
    ...
    if (std::filesystem::exists(destPath)) {
      callback(createErrorResponse(
          409, "File exists",
          "A model file with the new name already exists. Please delete it "
...
void ModelUploadHandler::deleteModel(
  ...
  try {
    ...
    if (sanitizedFilename.empty() || sanitizedFilename != modelName) {
      callback(
          createErrorResponse(400, "Invalid filename", "Invalid model name"));
      return;
    }
...
HttpResponsePtr
ModelUploadHandler::createErrorResponse(int statusCode,
