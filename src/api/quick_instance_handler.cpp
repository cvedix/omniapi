...
QuickInstanceHandler::getDefaultParams(const std::string &solutionType,
  ...
  if (type == "face_detection" || type == "face") {
    if (input == "file" || input == "video") {
      defaults["FILE_PATH"] = "/opt/edge_ai_api/videos/face.mp4";
      defaults["MODEL_PATH"] =
  ...
  // Updated to match actual file locations in /opt/edge_ai_api/models/det_cls/
  else if (type == "ba_crossline" || type == "crossline" ||
           type == "behavior_analysis") {
  ...
  // Updated to match actual config file name
    ...
    defaults["FILE_PATH"] = "/opt/edge_ai_api/videos/face.mp4";
    defaults["MODEL_PATH"] =
        "/opt/edge_ai_api/models/mask_rcnn/frozen_inference_graph.pb";
    defaults["MODEL_CONFIG_PATH"] =
        "/opt/edge_ai_api/models/mask_rcnn/"
        "mask_rcnn_inception_v2_coco_2018_01_28.pbtxt";
    defaults["LABELS_PATH"] = "/opt/edge_ai_api/models/coco_80classes.txt";
    if (output == "rtmp") {
      defaults["RTMP_URL"] = "rtmp://localhost:1935/live/stream";
    }
  }
