...
std::string FontUploadHandler::fonts_dir_ = "./fonts";

void FontUploadHandler::setFontsDirectory(const std::string &dir) {
  fonts_dir_ = dir;
}

std::string FontUploadHandler::getFontsDirectory() const { return fonts_dir_; }

bool FontUploadHandler::isValidFontFile(const std::string &filename) const {
  // Allow common font file extensions
  ...
  return hasSuffix(lower, ".ttf") || hasSuffix(lower, ".otf") ||
         hasSuffix(lower, ".woff") || hasSuffix(lower, ".woff2") ||
         hasSuffix(lower, ".eot") || hasSuffix(lower, ".ttc") ||
         hasSuffix(lower, ".otc") || hasSuffix(lower, ".pfb") ||
         hasSuffix(lower, ".pfm") || hasSuffix(lower, ".afm") ||
         hasSuffix(lower, ".bdf") || hasSuffix(lower, ".pcf") ||
         hasSuffix(lower, ".snf") || hasSuffix(lower, ".fnt");
}

std::string
FontUploadHandler::sanitizeFilename(const std::string &filename) const {
...
std::string
FontUploadHandler::sanitizeDirectoryPath(const std::string &dirPath) const {
  if (dirPath.empty()) {
    return "";
  }
...
std::string
FontUploadHandler::getDirectoryPath(const HttpRequestPtr &req) const {
...
std::string
FontUploadHandler::extractFontName(const HttpRequestPtr &req) const {
  // Try getParameter first (standard way - Drogon auto-extracts from path
  // pattern)
  std::string fontName = req->getParameter("fontName");

  // Fallback: extract from path if getParameter doesn't work
  if (fontName.empty()) {
    // Try both path() and getPath() methods
    // path() returns the full path including query string
    // getPath() returns path without query string
    std::string path = req->path();
    std::string pathWithoutQuery = req->getPath();

    // Use path() if available, otherwise use getPath()
    if (path.empty() && !pathWithoutQuery.empty()) {
    ...
    if (fontPos != std::string::npos) {
      ...
      fontName = path.substr(start, end - start);
    } else {
      // Fallback to old pattern /fonts/ for backward compatibility
      size_t fontsPos = path.find("/fonts/");
      if (fontsPos != std::string::npos) {
        size_t start = fontsPos + 7;        // length of "/fonts/"
        size_t end = path.find("?", start); // Stop at query string if present
        if (end == std::string::npos) {
          end = path.length();
        }
  ...
  // URL decode the font name if it contains encoded characters
  if (!fontName.empty()) {
    std::string decoded;
    decoded.reserve(fontName.length());
    for (size_t i = 0; i < fontName.length(); ++i) {
      if (fontName[i] == '%' && i + 2 < fontName.length()) {
        // Try to decode hex value
        char hex[3] = {fontName[i + 1], fontName[i + 2], '\0'};
        char *end;
        unsigned long value = std::strtoul(hex, &end, 16);
        if (*end == '\0' && value <= 255) {
          decoded += static_cast<char>(value);
          ...
          decoded += fontName[i]; // Invalid encoding, keep as-is
        }
      } else {
        decoded += fontName[i];
      }
...
void FontUploadHandler::uploadFont(
  ...
  try {
    ...
    if (isMultipart) {
      ...
      // Ensure fonts directory exists (with fallback if needed)
      std::string fontsDir = getFontsDirectory();
      fontsDir = EnvConfig::resolveDirectory(fontsDir, "fonts");
      ...
      while (partStart != std::string::npos) {
        ...
        if (!isValidFontFile(partFilename)) {
        ...
        std::filesystem::path partFilePath = fontsPath / sanitizedPartFilename;
        ...
        if (std::filesystem::exists(partFilePath)) {
          std::filesystem::path basePath = fontsPath / sanitizedPartFilename;
          ...
          do {
            ...
            partFilePath = fontsPath / finalFilename;
            counter++;
        ...
        try {
          ...
          fileCount++;
          std::cerr << "[FontUploadHandler] Font file uploaded: "
      ...
      if (fileCount > 0) {
        ...
        response["message"] =
            "Uploaded " + std::to_string(fileCount) + " font file(s)";
      ...
      if (originalFilename.empty()) {
        originalFilename = "uploaded_font.ttf";
      }

      // Validate file extension
      if (!isValidFontFile(originalFilename)) {
        callback(createErrorResponse(400, "Invalid file type",
                                     "Only font files (.ttf, .otf, .woff, "
                                     ".woff2, .eot, .ttc, etc.) are allowed"));
        return;
      }
      ...
      // Create full file path - if file exists, add number to name
      filePath = fontsPath / sanitizedFilename;

      // If file already exists, find a unique name by adding number
      if (std::filesystem::exists(filePath)) {
        std::filesystem::path basePath = fontsPath / sanitizedFilename;
        std::string baseName =
            basePath.stem().string(); // filename without extension
        std::string extension =
            basePath.extension().string(); // .ttf, .otf, etc.

        int counter = 1;
        std::string newFilename;
        do {
          newFilename = baseName + "_" + std::to_string(counter) + extension;
          filePath = fontsPath / newFilename;
          counter++;
        ...
        std::cerr
            << "[FontUploadHandler] File with original name exists, using: "
    ...
    try {
      ...
      try {
        ...
        std::cerr << "[FontUploadHandler] Error getting file size: " << e.what()
                  << std::endl;
      ...
      try {
        ...
        std::cerr << "[FontUploadHandler] Error getting canonical path: "
                  << e.what() << std::endl;
      ...
      std::cerr << "[FontUploadHandler] Unexpected error: " << e.what()
                << std::endl;
      callback(createErrorResponse(
    ...
    Json::Value response;
    response["success"] = true;
    response["message"] = "Font file uploaded successfully";
...
void FontUploadHandler::listFonts(
  ...
  try {
    ...
    if (subDir.empty()) {
      // Recursive search: find all font files in all subdirectories
      try {
        for (const auto &entry :
             std::filesystem::recursive_directory_iterator(fontsPath)) {
          try {
            if (entry.is_regular_file() &&
                isValidFontFile(entry.path().filename().string())) {
              Json::Value font;
              font["filename"] = entry.path().filename().string();

              // Get relative path from base fonts directory
              try {
                std::filesystem::path relativePath =
                    std::filesystem::relative(entry.path(), fontsPath);
                font["relativePath"] = relativePath.string();
              } catch (const std::exception &e) {
                // If relative path fails, use filename only
                font["relativePath"] = entry.path().filename().string();
              }

              font["path"] = std::filesystem::canonical(entry.path()).string();
              font["size"] = static_cast<Json::Int64>(
            ...
            // etc.)
            std::cerr << "[FontUploadHandler] Skipping entry due to error: "
                      << e.what() << std::endl;
            continue;
          }
        }
      } catch (const std::filesystem::filesystem_error &e) {
        std::cerr << "[FontUploadHandler] Error during recursive search: "
                  << e.what() << std::endl;
    ...
    response["directory"] = std::filesystem::canonical(fontsPath).string();
...
void FontUploadHandler::renameFont(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback) {

  try {
    // Debug: log request details for troubleshooting
    std::cerr << "[FontUploadHandler::renameFont] Path: " << req->path()
              << ", getPath(): " << req->getPath() << std::endl;
    std::cerr << "[FontUploadHandler::renameFont] getParameter('fontName'): "
              << req->getParameter("fontName") << std::endl;

    // Get font name from path parameter
    std::string fontName = extractFontName(req);
    std::cerr << "[FontUploadHandler::renameFont] Extracted fontName: "
              << fontName << std::endl;

    if (fontName.empty()) {
      callback(
          createErrorResponse(400, "Invalid request", "Font name is required"));
      return;
    }

    // Sanitize source filename
    std::string sanitizedSourceName = sanitizeFilename(fontName);
    if (sanitizedSourceName.empty() || sanitizedSourceName != fontName) {
      callback(createErrorResponse(400, "Invalid filename",
                                   "Invalid source font name"));
      return;
    }
    ...
    if (!isValidFontFile(newName)) {
      callback(
          createErrorResponse(400, "Invalid file type",
                              "New name must have a valid font file extension "
    ...
    std::filesystem::path sourcePath = fontsPath / sanitizedSourceName;
    std::filesystem::path destPath = fontsPath / sanitizedNewName;

    // Check if source file exists
    if (!std::filesystem::exists(sourcePath)) {
      callback(
          createErrorResponse(404, "Not found", "Source font file not found"));
      return;
    }
    ...
    if (std::filesystem::exists(destPath)) {
      callback(createErrorResponse(
          409, "File exists",
          "A font file with the new name already exists. Please delete it "
    ...
    Json::Value response;
    response["success"] = true;
    response["message"] = "Font file renamed successfully";
...
void FontUploadHandler::deleteFont(
  ...
  try {
    ...
    // Sanitize filename
    std::string sanitizedFilename = sanitizeFilename(fontName);
    if (sanitizedFilename.empty() || sanitizedFilename != fontName) {
      callback(
          createErrorResponse(400, "Invalid filename", "Invalid font name"));
      return;
    }
    ...
    std::filesystem::path filePath = fontsPath / sanitizedFilename;

    // Check if file exists
    if (!std::filesystem::exists(filePath)) {
      callback(createErrorResponse(404, "Not found", "Font file not found"));
      return;
    }
    ...
    if (!std::filesystem::remove(filePath)) {
      callback(createErrorResponse(500, "Delete failed",
                                   "Could not delete font file"));
      return;
    }

    Json::Value response;
    response["success"] = true;
    response["message"] = "Font file deleted successfully";
    response["filename"] = sanitizedFilename;
...
void FontUploadHandler::handleOptions(
  ...
  std::cerr << "[FontUploadHandler] Path: " << req->path() << std::endl;
  std::cerr << "[FontUploadHandler] Origin: " << req->getHeader("Origin")
            << std::endl;
...
HttpResponsePtr
FontUploadHandler::createErrorResponse(int statusCode, const std::string &error,
