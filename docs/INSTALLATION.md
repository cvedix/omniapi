# Hướng Dẫn Cài Đặt omniapi

Hướng dẫn chi tiết về cách build và cài đặt omniapi từ Debian package (.deb).
...
- Nếu cài đặt OpenCV bị lỗi hoặc bị gián đoạn, chạy lại: `sudo /opt/omniapi/scripts/build_opencv_safe.sh`
...
**Quản lý service:**
```bash
sudo systemctl start omniapi      # Khởi động
sudo systemctl stop omniapi       # Dừng
sudo systemctl restart omniapi    # Khởi động lại
sudo systemctl status omniapi     # Kiểm tra trạng thái
```

**Cấu trúc sau khi cài đặt:**
- **Executable**: `/usr/local/bin/omniapi`
- **Libraries**: `/opt/omniapi/lib/` (bundled - tự chứa)
- **GStreamer plugins**: `/opt/omniapi/lib/gstreamer-1.0/`
- **Config**: `/opt/omniapi/config/`
- **Data**: `/opt/omniapi/` (instances, solutions, models, logs, etc.)
- **Fonts**: `/opt/omniapi/fonts/` (default fonts)
- **Models**: `/opt/omniapi/models/` (default models)
...
sudo /opt/omniapi/scripts/fix_rpath.sh

# Hoặc fix thủ công
sudo patchelf --set-rpath "/opt/omniapi/lib:/opt/cvedix/lib" /usr/local/bin/omniapi
sudo patchelf --set-rpath "/opt/omniapi/lib:/opt/cvedix/lib" /usr/local/bin/edgeos-worker

# Verify RPATH đã được fix
patchelf --print-rpath /usr/local/bin/omniapi
# Kết quả mong đợi: /opt/omniapi/lib:/opt/cvedix/lib
