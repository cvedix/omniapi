...
**PATCH with only CrossingLines (line-only update):** When the request body contains only `AdditionalParams.CrossingLines` (and no other structural or param changes), the worker treats it as a **line-only** update: it applies lines via `set_lines()` at runtime and does **not** perform a hot-swap or pipeline restart. The RTMP output stream therefore stays continuous with no reconnection. The worker syncs `AdditionalParams` ↔ `additionalParams` so that a PATCH sent with PascalCase `AdditionalParams` only still updates both representations and is correctly detected as line-only. Test script: `scripts/test_zero_downtime_patch_lines.sh`.
